...
export class PlannerHiringDatasourceImpl implements PlannerHiringDatasource {
  public async getPlannerHiringById(getByIdDto: GetPlannerHiringByIdDto): Promise<PlannerHiringSummaryEntity> {
    ...
    const directorate = await this.getDirectorateByCode(plannerHiring.directorateCodeReference);
    const managementArea = await this.getManagementAreaByCode(plannerHiring.managementAreaCodeReference);
    const costCenter = await this.getCostCenterByCode(plannerHiring.costCenterCodeReference);

    return PlannerHiringSummaryEntity.fromJson({
      id: plannerHiring.id,
      directorate,
      managementArea,
      costCenter,
  ...
  public async createPlannerHiring(createDto: CreatePlannerHiringDto): Promise<PlannerHiringSummaryEntity> {
    ...
    if (planner.plannerHiring?.id) {
      throw AppError.badRequest(
        `Ya existe una etapa de contratación de honorarios para la planificación con id ${plannerId} no existe`,
      );
    }

    // TODO: validate planner connected

    const createdPlannerHiring = await prisma.plannerHiring.create({
      data: {
    ...
    const directorate = await this.getDirectorateByCode(createdPlannerHiring.directorateCodeReference);
    const managementArea = await this.getManagementAreaByCode(createdPlannerHiring.managementAreaCodeReference);
    const costCenter = await this.getCostCenterByCode(createdPlannerHiring.costCenterCodeReference);

    return PlannerHiringSummaryEntity.fromJson({
      id: createdPlannerHiring.id,
      directorate,
      managementArea,
      costCenter,
    });
